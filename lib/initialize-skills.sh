...
SKILLS_DIR="${HOME}/.config/resonix/skills"
SKILLS_REPO="https://github.com/resonix/skills.git"
...
if [ -d "${HOME}/.config/resonix/.git" ]; then
    echo "Found existing installation. Backing up..."
    mv "${HOME}/.config/resonix/.git" "${HOME}/.config/resonix/.git.bak"

    if [ -d "${HOME}/.config/resonix/skills" ]; then
        mv "${HOME}/.config/resonix/skills" "${HOME}/.config/resonix/skills.bak"
        echo "Your old skills are in ~/.config/resonix/skills.bak"
    fi
fi

# Clone the skills repository
mkdir -p "${HOME}/.config/resonix"
...
if command -v gh &> /dev/null; then
    echo ""
    echo "GitHub CLI detected. Would you like to fork resonix-skills?"
    echo "Forking allows you to share skill improvements with the community."
    echo ""
    read -p "Fork resonix-skills? (y/N): " -n 1 -r
    echo

    if [[ $REPLY =~ ^[Yy]$ ]]; then
        gh repo fork resonix/skills --remote=true
